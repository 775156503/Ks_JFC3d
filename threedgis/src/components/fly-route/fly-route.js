...
import { storeState } from './js/store' //简单局部状态管理
import tool from './js/tool.js' //提示等工具
import createFlyLine_xml from './fly-line-xml.js'
import createTooltip from './js/tooltips2'

const emit = defineEmits(['closeFly2'])
const closeFly2 = () => {
  emit('closeFly2', false)
}
    ...
    (val) => {
      if (val === 'customRoute') {
        window.tooltip.showAt(
          ' <p>调整当前相机位置和视角</p><p>以当前相机位置和视角设置站点</p><p>点击添加保存此站点</p>',
          '50%'
        )
        setTimeout(() => {
