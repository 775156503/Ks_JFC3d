...
Dialog.prototype.setPosition = function () {
    ...
    $(thisdialogDiv).animate({  opacity:'1'}, 1000);

    thisdialogDiv.style.top = Math.round(thisTop) + "px";
    thisdialogDiv.style.left = Math.round(thisLeft) + "px";
    this.Left = Math.round(thisLeft);
    this.Top = Math.round(thisTop);
    //thisdialogDiv.slideToggle(6000);
};
...
Dialog.prototype.create = function () {
    ...
    var DialogDivWidth = this.Width + 3 + 3;
    var DialogDivHeight = this.Height + 33 + 13 + (this.ShowButtonRow ? 40 : 0) + (this.ShowMessageRow ? 50 : 0);
    if (DialogDivWidth > bd.clientWidth) this.Width = Math.round(bd.clientWidth - 6);
    if (DialogDivHeight > bd.clientHeight) this.Height = Math.round(bd.clientHeight - 46 - (this.ShowButtonRow ? 40 : 0) - (this.ShowMessageRow ? 50 : 0));

    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 6) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:15px; line-height:1.4;background-color: white;border-radius: 5px;box-shadow: 0 0 8px 0 rgba(0, 0, 0, 0.2);">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "border-bottom: 1px solid #6ED0E0;box-shadow: 0 0 8px 0 rgba(0, 0, 0, 0.2);border:none;background-image: linear-gradient(to right, #217DD9, #4DB5FF) !important;" : "") + '">\
     <td height="40" style="border-bottom: 2px solid #548DC6;border:none;"><div style="float: left; font-weight: bold; color:#fff; margin-left: 6px;"><span id="_Title_' + this.ID + '" style="color:#fff;font-size: 18px;">' + this.Title + '</span></div>\
    ...
    <tr valign="top">\
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          <tr id="_MessageRow_' + this.ID + '" style="' + (this.ShowMessageRow ? "" : "display:none") + '">\
            <td valign="top" height="50"><table width="100%" cellspacing="0" cellpadding="0" border="0" style="" id="_MessageTable_' + this.ID + '">\
                <tr>\
	    ...
	    return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="buttonStyle jimu-btn" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
                <input type="button" class="buttonStyle jimu-btn" value="取 消" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
    </tr>\
...
Dialog.prototype.setSize = function (w, h) {
    if (w && +w > 20) {
        this.Width = +w;
        topWin.$id("_DialogTable_" + this.ID).width = this.Width + 6;
...
Dialog.prototype.show = function () {
    ...
    var bgdiv = Dialog.getBgdiv(),
		thisdialogDiv = this.getDialogDiv();
    this.zindex = thisdialogDiv.style.zIndex =parseInt( Dialog.bgDiv.style.zIndex )+2;
    if (topWin.Dialog._Array.length > 0) {
        this.zindex = thisdialogDiv.style.zIndex = parseInt(topWin.Dialog._Array[topWin.Dialog._Array.length - 1].zindex) + 2 + 1;
    ...
    if (this.Modal) {
        //bgdiv.style.zIndex = topWin.Dialog._Array[topWin.Dialog._Array.length - 1].zindex;
...
Dialog.prototype.close = function () {
    ...
    $(thisdialogDiv).animate({  opacity: '0.1',height:'50px',width:'50px' }, 500);
    setTimeout(function () {
...
Dialog.getBgdiv = function () {
    ...
    if (!bgdiv) {
        ...
        bgdiv.style.cssText = "position:absolute;left:0px;top:0px;width:100%;height:100%;z-index:9999;display:block;";
...
Dialog.alert = function (msg, func, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:16px;padding-left: 10px;">' + msg + '</td></tr>\
	</table>';
    diag.show();
    //Dialog.bgDiv.style.display = "none";
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + '信息.png" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:9pt">' + msg + '</td></tr>\
	</table>';
    Dialog.bgDiv.style.display = "block";
