 ...
 */
    ...
    var StylePanel = function(div,plotEditControl,plotting) {
        ...
        this._displayTextContentName = ["注记内容", "注记位置", "字体背景颜色", "注记大小", "注记字体", "注记颜色", "字体边框"];
        ...
        this._plottingEdit.SelectedEvent.addEventListener(function(geoGraphicObject) {
            ...
            _self.updatePropertyGrid();
            // _self.tabs.tabs("select", "基本");
        });

        // 选中标号被释放回调函数
        this._plottingEdit.UnSelectedEvent.addEventListener(function() {
            _self.releaseSelection();
        });
    ...
    StylePanel.prototype.releaseSelection = function() {
    ...
    StylePanel.prototype.init = function() {
        ...
        _self.tabs = styleTabs;

        styleTabs.tabs('add', {
            title : "基本",
            content : '<div id="basicPanel" style="height: 100%"></div>',
            closable : false
        });

        styleTabs.tabs('add', {
            title : "风格",
            content : '<div id="mannerPanel" style="height: 100%"></div>',
            closable : false
        });

        styleTabs.tabs('add', {
            title : "位置",
            content : '<div id="locationPanel" style="height: 100%"></div>',
            closable : false
        });

        styleTabs.tabs('add', {
            title : "文本",
            content : '<div id="textPanel" style="height: 100%"></div>',
            closable : false
        });

        var basicPanel = $('#basicPanel');
        this._basicPanel = basicPanel;
        this.initBasicPanel();

        var mannerPanel = $('#mannerPanel');
        this._mannerPanel = mannerPanel;
        this.initMannerPanel();

        var positionPanel = $('#locationPanel');
        ...
        // _self.initBasicPanel();
        // _self.initMannerPanel();
        // _self.initPositionPanel();
        // _self.initTextPanel();

    };

    StylePanel.prototype.initBasicPanel = function() {

        var _self = this;

        function afterModifySelectFeatureInBasicPanel() {
            var updated = $('#basicpg').propertygrid('getChanges', "updated");
            if (updated.length !== 0) {
                _self.updateSelectFeature(updated[0], _self._selectedFeature);
            }
            var rows = _self.collectionBasicPropertyGridRows(_self._selectedFeature);
            $('#basicpg').propertygrid('loadData', rows);
        }

        var baiscPanel = _self._basicPanel;

        var propertygrid = document.createElement('table');
        propertygrid.id = "basicpg";
        propertygrid.className = "easyui-propertygrid";
        baiscPanel.append(propertygrid);

        $('#basicpg').propertygrid({
            showGroup : true,
            columns : [[
                { field: 'name', title: 'Name', width: 100, resizable: true },
                { field: 'value', title: 'Value', width: 100, resizable: true }
            ]],
            onAfterEdit : afterModifySelectFeatureInBasicPanel
        });
    };

    StylePanel.prototype.initMannerPanel = function() {

        var _self = this;

        function afterModifySelectFeatureInMannerPanel() {
            var updated = $('#mannerpg').propertygrid('getChanges', "updated");
            if (0 !== updated.length) {
                _self.updateSelectFeature(updated[0], _self._selectedFeature);
            }
            var rows = _self.collectionMannerPropertyGridRows(_self._selectedFeature);
            $('#mannerpg').propertygrid('loadData', rows);
        }

        var mannerPanel = _self._mannerPanel;

        var propertygrid = document.createElement('table');
        propertygrid.id = "mannerpg";
        propertygrid.className = "easyui-propertygrid";
        mannerPanel.append(propertygrid);

        $('#mannerpg').propertygrid({
            showGroup : true,
            columns : [[
                { field: 'name', title: 'Name', width: 100, resizable: true },
                { field: 'value', title: 'Value', width: 100, resizable: true }
            ]],
            onAfterEdit : afterModifySelectFeatureInMannerPanel
        });
    };

    StylePanel.prototype.initPositionPanel = function() {

        var _self = this;

        function afterModifySelectFeatureInPositionPanel() {
            var updated = $('#pospg').propertygrid('getChanges', "updated");
            if (0 !== updated.length) {
                _self.updateSelectFeature(updated[0], _self._selectedFeature);
            }
            var rows = _self.collectionLocationPropertyGridRows(_self._selectedFeature);
            $('#pospg').propertygrid('loadData', rows);
        }

        var positionPanel = _self._positionPanel;

        var propertygrid = document.createElement('table');
        propertygrid.id = "pospg";
        propertygrid.className = "easyui-propertygrid";
        positionPanel.append(propertygrid);
        ...
        $('#pospg').propertygrid({
            ...
            onAfterEdit : afterModifySelectFeatureInPositionPanel
        });
    };


    StylePanel.prototype.initTextPanel = function() {

        var _self = this;

        function afterModifySelectFeatureInTextPanel() {
            var updated = $('#txtpg').propertygrid('getChanges', "updated");

            if (0 !== updated.length) {
                _self.updateSelectFeature(updated[0], _self._selectedFeature);
            }

            var rows = _self.collectionTextPropertyGridRows(_self._selectedFeature);
            $('#txtpg').propertygrid('loadData', rows);
        }

        var textPanel = _self._textPanel;

        var propertygrid = document.createElement('table');
        propertygrid.id = "txtpg";
        propertygrid.className = "easyui-propertygrid";
        textPanel.append(propertygrid);
        ...
        $('#txtpg').propertygrid({
            ...
            onAfterEdit : afterModifySelectFeatureInTextPanel
        });
    };


    StylePanel.prototype.collectionBasicPropertyGridRows = function(graphicObject) {
        ...
        if (null !== graphicObject && undefined !== graphicObject) {
            ...
            if (this._selectedFeature._symbolType === 1) {
                ...
                gridSymbolSizeXObj.value = this._selectedFeature.gridSymbolSize.x;
                ...
                gridSymbolSizeYObj.value = this._selectedFeature.gridSymbolSize.y;
    ...
    StylePanel.prototype.collectionMannerPropertyGridRows = function(graphicObject) {
        ...
        lineWidthObj.value = 1===graphicObject.symbolType?this._selectedFeature.gridLineWidth : this._selectedFeature.symbolStyle.lineWidth;
    ...
    StylePanel.prototype.collectionLocationPropertyGridRows = function(graphicObject) {
        ...
        var _self = this;
        var selectedFeature = _self._selectedFeature;
    ...
    StylePanel.prototype.collectionTextPropertyGridRows = function(graphicObject) {
        ...
        fontHaloObj.value = this.checkboxValueToString(this._selectedFeature.markHalo);
        fontHaloObj.group = this._group[5];
        fontHaloObj.editor = {"type": "checkbox", "options": {"on": true, "off": false}};

        if (1 === selectedFeature.symbolType) {
            if (0 === selectedFeature.showMode ||  1 === selectedFeature.showMode ||  2 === selectedFeature.showMode) {
        ...
        if (null != updated) {
            switch (updated.name) {
                ...
                case this._displayLineStyleName[0]:
                    if (1===selectFeature.symbolType) {
                        selectFeature.gridLineWidth = parseInt(updated.value);
                    } else {
                        selectFeature.symbolStyle.lineWidth = parseInt(updated.value);
                    }
                    break;
    ...
    StylePanel.prototype.updatePropertyGrid = function(type) {
        var _self = this;
        if(1 === type){
            var basicRows = _self.collectionBasicPropertyGridRows(undefined);
            $('#basicpg').propertygrid('loadData', basicRows);
            var mannerRows = _self.collectionMannerPropertyGridRows(undefined);
            $('#mannerpg').propertygrid('loadData', mannerRows);
            var posRows = _self.collectionLocationPropertyGridRows(undefined);
            $('#pospg').propertygrid('loadData', posRows);
            var textRows = _self.collectionTextPropertyGridRows(undefined);
            $('#txtpg').propertygrid('loadData', textRows);
        }else {
            var basicRows = _self.collectionBasicPropertyGridRows(_self._selectedFeature);
            $('#basicpg').propertygrid('loadData', basicRows);
            var mannerRows = _self.collectionMannerPropertyGridRows(_self._selectedFeature);
            $('#mannerpg').propertygrid('loadData', mannerRows);
            var posRows = _self.collectionLocationPropertyGridRows(_self._selectedFeature);
            $('#pospg').propertygrid('loadData', posRows);
            var textRows = _self.collectionTextPropertyGridRows(_self._selectedFeature);
            $('#txtpg').propertygrid('loadData', textRows);
        }
    };
    ...
    StylePanel.prototype.getDisplayRows = function(graphicObject) {
        var rows = [];
    ...
    StylePanel.prototype.getArrowTypeRows = function(graphicObject) {
        var rows = [];
    ...
    StylePanel.prototype.getTextTypeRows = function(graphicObject) {
    ...
    StylePanel.prototype.getArrowHeadTypeRows = function(graphicObject) {

        var rows = [];

        rows.push({"value" : "0", "text" : "双线箭头"});
        rows.push({"value" : "1", "text" : "五箭头"});
    ...
    StylePanel.prototype.getArrowBodyTypeRows = function(graphicObject) {
    ...
    StylePanel.prototype.getArrowTailTypeRows = function(graphicObject) {
